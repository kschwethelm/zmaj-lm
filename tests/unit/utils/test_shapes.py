...
class TestSplitHeads:
    ...
    def test_shape(self, device: torch.device) -> None:
        ...
        n_heads = 12
        x = torch.ones((batch, seq_len, d_model), device=device)

        result = split_heads(x, n_heads)

        assert result.shape == (batch, seq_len, n_heads, d_model // n_heads)

    def test_values_preserved(self, device: torch.device) -> None:
        """Test that split_heads preserves values, only reshapes."""
        batch, seq_len, d_model = 2, 4, 8
        n_heads = 2
        # Create array with unique values
        x = torch.arange(batch * seq_len * d_model, device=device).reshape(batch, seq_len, d_model)

        result = split_heads(x, n_heads)

        # Values should be preserved, just reshaped
        assert torch.equal(result.reshape(batch, seq_len, d_model), x)
    ...
    def test_indivisible_raises_assertion(self, device: torch.device) -> None:
...
class TestSplitHeadsTransposed:
    ...
    def test_transpose_order(self, device: torch.device) -> None:
        ...
        expected = split_heads(x, n_heads).transpose(1, 2)
    ...
    def test_values_preserved(self, device: torch.device) -> None:
        ...
        recovered = result.transpose(1, 2).reshape(batch, seq_len, d_model)
...
class TestMergeHeads:
    ...
    def test_shape(self, device: torch.device) -> None:
        ...
        x = torch.ones((batch, seq_len, n_heads, d_head), device=device)

        result = merge_heads(x)

        assert result.shape == (batch, seq_len, n_heads * d_head)

    def test_round_trip_with_split(self, device: torch.device) -> None:
        """Test that split_heads -> merge_heads is identity."""
        batch, seq_len, d_model = 4, 128, 768
        n_heads = 12
        x = torch.arange(batch * seq_len * d_model, dtype=torch.float32, device=device).reshape(
...
class TestMergeHeadsTransposed:
    ...
    def test_shape(self, device: torch.device) -> None:
        ...
        x = torch.ones((batch, n_heads, seq_len, d_head), device=device)

        result = merge_heads_transposed(x)

        assert result.shape == (batch, seq_len, n_heads * d_head)

    def test_round_trip_with_split_transposed(self, device: torch.device) -> None:
    ...
    def test_transpose_correctness(self, device: torch.device) -> None:
        """Test that merge_heads_transposed correctly untransposes."""
        batch, n_heads, seq_len, d_head = 2, 3, 4, 5
        # Create array with unique values
        x = torch.arange(batch * n_heads * seq_len * d_head, device=device).reshape(
        ...
        expected = x.transpose(1, 2).reshape(batch, seq_len, n_heads * d_head)
        assert torch.equal(result, expected)
...
class TestAssertShape:
    ...
    def test_correct_shape_no_error(self, device: torch.device) -> None:
        """Test that correct shape raises no error."""
        x = torch.ones((4, 128, 768), device=device)
        # Should not raise
        assert_shape(x, (4, 128, 768), "test_tensor")

    def test_wildcard_dimensions(self, device: torch.device) -> None:
    ...
    def test_wrong_dimension_raises_error(self, device: torch.device) -> None:
    ...
    def test_wrong_number_of_dimensions_raises_error(self, device: torch.device) -> None:
    ...
    def test_error_message_includes_tensor_name(self, device: torch.device) -> None:
        """Test that error message includes the tensor name."""
        x = torch.ones((4, 128), device=device)
...
class TestShapeStr:
    ...
    def test_with_name(self, device: torch.device) -> None:
        """Test shape_str formatting with name."""
        x = torch.ones((4, 128, 768), dtype=torch.float32, device=device)
        result = shape_str(x, "query")

        assert "query:" in result
        assert "(4, 128, 768)" in result or "torch.Size([4, 128, 768])" in result
        assert "float32" in result

    def test_without_name(self, device: torch.device) -> None:
    ...
    def test_different_dtypes(self, device: torch.device) -> None:
        """Test that shape_str includes dtype information."""
        x_f32 = torch.ones((4, 128), dtype=torch.float32, device=device)
        x_f16 = torch.ones((4, 128), dtype=torch.float16, device=device)
        x_int = torch.ones((4, 128), dtype=torch.int32, device=device)
...
class TestIntegrationScenarios:
    ...
    def test_attention_flow_non_transposed(self, device: torch.device) -> None:
        """Test typical flow: project -> split -> attend -> merge."""
        batch, seq_len, d_model, n_heads = 2, 4, 8, 2
        d_head = d_model // n_heads

        # Simulate Q, K, V projections
        q = torch.ones((batch, seq_len, d_model), device=device)
        ...
        assert torch.equal(q_merged, q)

    def test_attention_flow_transposed(self, device: torch.device) -> None:
    ...
    def test_shape_validation_in_pipeline(self, device: torch.device) -> None:
