...
class TestTokenizerTrainingConfig:
    ...
    def test_vocab_size_minimum_validation(self) -> None:
        ...
        with pytest.raises(ValueError, match="vocab_size .* must be at least 256"):
            TokenizerTrainingConfig(training_corpus_path="/path/to/corpus", vocab_size=255)

        with pytest.raises(ValueError, match="vocab_size .* must be at least 256"):
            TokenizerTrainingConfig(training_corpus_path="/path/to/corpus", vocab_size=100)

        # Should succeed with vocab_size >= 256
        config = TokenizerTrainingConfig(training_corpus_path="/path/to/corpus", vocab_size=256)
    ...
    def test_valid_vocab_sizes(self) -> None:
        ...
        for size in valid_sizes:
            config = TokenizerTrainingConfig(
                training_corpus_path="/path/to/corpus", vocab_size=size
            )
    ...
    def test_special_tokens_computed_field(self) -> None:
        ...
        config = TokenizerTrainingConfig(training_corpus_path="/path/to/corpus")
        ...
        config_custom = TokenizerTrainingConfig(
            training_corpus_path="/path/to/corpus",
            pad_token="[PAD]",
            bos_token="[BOS]",
            eos_token="[EOS]",
            unk_token="[UNK]",
    ...
    def test_serialization_roundtrip(self) -> None:
        ...
        config = TokenizerTrainingConfig(
            training_corpus_path="/data/corpus.txt",
            vocab_size=16_000,
            min_frequency=3,
        ...
        assert restored.training_corpus_path == "/data/corpus.txt"
